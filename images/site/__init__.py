...
def create_app(test_config=None):
    ...
    def get_images(album_id):
        images = [image for image in os.listdir(os.path.join(app.static_folder, f"albums/{album_id}")) if os.path.splitext(image)[1].lower() != ".json"]
        return images

    def random_image(album_id, homepage=False):
        if homepage:
    ...
    def home():
        ...
        return render_template('home.html', albums=albums, random_image = random_image, get_json = get_json)

    @app.route('/a/<album_id>')
    def album(album_id):
        with open(os.path.join(app.static_folder, f"albums/{album_id}/album.json"), "r") as file:
            json_file = json.load(file)
        return render_template('album.html', album_id = album_id, json_file = json_file, remove_extension = remove_extension, random_image = random_image) 
